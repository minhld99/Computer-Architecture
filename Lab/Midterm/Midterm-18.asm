...
input: .space 32			# save user input
start: .asciiz "\n\n----------- Let's start -----------"
request: .asciiz "\nInput: "
...
init:	li  	$v0, 4     		# print_string call number
	...
	li  	$s0, 0      		# i = 0 run through all text
    	...
    	li	$s4, 0			# curren longest length of result
    	li	$s6, 0			# k = 0: index to clear tmp 
	li	$t0, 32			# assume max length = 32 characters
	la	$a2, tmp		# load tmp address
	
main: 	jal	checkNonAlpha		# loop through all characters
...
continue:	
	j 	readEnter 		# press enter to continue
...
#################################################################################################
checkNonAlpha:
	beq	$s0, $t0, done		# if i = 32 end
	add	$t4, $s0, $a0		# address of Example[i] in $t4
	lb	$s1, 0($t4)		# load value of Example[i]
...
#########################################################################################
checkLength:
	slt 	$t3, $s2, $s4		# if length of current word is not longer than current max
	bnez 	$t3, next		# reset j and move to the next one
	add	$s4, $zero, $s2		# else, we have new longest length
next:	addi	$s0, $s0, 1		# i = i + 1
	li	$s2, 0			# j = 0
...
#################################################################################################
printWord:
	beq	$s0, $t0, continue	# if i = 32 end -> press enter to continue
	add	$t4, $s0, $a1		# address of Example[i] in $t4
	lb	$s1, 0($t4)		# load value of Example[i]
